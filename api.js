...
export function constructApi1URL(assets2) {
  ...
  const valuesAPIkey =
    "?access_key=6oemg05g8508q5s2rrzn2aqjv4ig7ac7t4oz04u0bukj500tq02t6jmwhits";
  ...
  for (let singleAsset in assets2) {
    if ((assets2[singleAsset].assetAbb = undefined)) {
      return;
    }
  ...
  const valuesAPIlink =
    valuesAPIurl + valuesAPIkey + valuesAPIbase + valuesAPIsymbols;
      ...
      try {
        let response = await fetch(
          "https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=" +
            assets2[index].assetAbb +
            "&apikey=Z1EVIMF28V618X3D"
        );
        let assetValueFromApi2 = await response.json();
        assetValueFromApi2 = await assetValueFromApi2["Global Quote"][
          "05. price"
        ];
